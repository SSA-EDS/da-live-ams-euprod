...
describe('Libs', () => {
  it('Default Libs', () => {
    const libs = setNx('/nx');
    expect(libs).to.equal('https://main--da-nx-ams-1128-west--ssa-eds.eumseds.live/nx');
  });

  it('Default NX on production / CDN', () => {
    const location = { hostname: 'eumseds-da.live' };
  ...
  it('Supports NX query param on production', () => {
    const location = {
      hostname: 'eumseds-da.live',
      search: '?nx=foo',
    };
    const libs = setNx('/nx', location);
    expect(libs).to.equal('https://foo--da-nx-ams-1128-west--ssa-eds.eumseds.live/nx');
  });
